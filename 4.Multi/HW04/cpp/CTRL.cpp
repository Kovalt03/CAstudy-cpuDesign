...
void CTRL::nextState(uint32_t opcode, uint32_t funct) {
    switch (state) {
        case STATE_IF:
            state = STATE_ID;
            break;
        case STATE_ID:
            ...
            break;
        case STATE_EX:
			if(opcode == OP_BEQ || opcode == OP_BNE || opcode == OP_J || (opcode == OP_RTYPE && funct == FUNCT_JR)){
				state = STATE_IF;
			}else if(opcode == OP_SW || opcode == OP_LW){
				state = STATE_MEM;
			}else if(opcode == OP_RTYPE || opcode == OP_ADDIU || opcode == OP_LUI || opcode == OP_ORI || opcode == OP_ANDI || opcode == OP_SLTI || opcode == OP_XORI || opcode == OP_SLTIU || opcode == OP_JAL){
				state = STATE_WB;
			}else{
				state = STATE_MEM;
			}
        ...
        case STATE_MEM:
            ...
            break;
        case STATE_WB:
            state = STATE_IF; // 다시 IF로 돌아가면서 반복
            break;
        default:
            state = STATE_ERROR; // 나머지는 ERROR
            break;
    }
	// printf("%->%d/",state);
}
	...
	switch(state){
		...
		case STATE_WB:
			if(opcode==OP_RTYPE){
				...
				controls->SavePC = 1;
				controls->RegWrite = 1;
			}else if(opcode==OP_LUI || opcode==OP_ORI || opcode==OP_ANDI || opcode==OP_SLTI || opcode==OP_XORI || opcode==OP_ADDIU || opcode==OP_SLTIU){
				controls->RegWrite = 1;
				controls->MemtoReg = 0;
			}
			break;
