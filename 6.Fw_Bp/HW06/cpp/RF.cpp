...
void RF::read(uint32_t rd_addr1, uint32_t rd_addr2, uint32_t *rd_data1, uint32_t *rd_data2, uint32_t wr_addr, uint32_t wr_data) {
	//if (rd_addr1 < REGSIZE)
	if(wr_addr == rd_addr1) *rd_data1 = wr_data;
	else *rd_data1 = register_files[rd_addr1];

	if(wr_addr == rd_addr2) *rd_data2 = wr_data;
	else *rd_data2 = register_files[rd_addr2];
...
void RF::write(uint32_t wr_addr, uint32_t wr_data, uint32_t RegWrite) {
	//if (RegWrite && wr_addr < REGSIZE) {
	if (RegWrite && wr_addr != 0) {
        register_files[wr_addr] = wr_data;
    } else if (wr_addr == 0) {
		register_files[wr_addr] = 0; //$zero 레지스터값 유지
	}
}
