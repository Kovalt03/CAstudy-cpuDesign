    ...
    uint32_t CPU::tick() {
        ...
        // IF
        uint32_t PC_next = PC;
        uint32_t predict_PC = bp.predict(PC);
        if (stall) {
        ...
        if (stall) {
            ...
            if (IF_ID_curr.inst != 0) {
                ...
                rf.read(parsed_inst.rs, parsed_inst.rt, &rs_data, &rt_data, wb_wr_addr, wb_wr_data);
                
                ID_EX_next.bh_PC = IF_ID_curr.bh_PC;
        ...
        // EX
        uint32_t forwarded_rt;
        // forwarding
        fwd = forward.Forwarding(ID_EX_curr.rs, ID_EX_curr.rt, EX_MEM_curr, MEM_WB_curr);
        switch (fwd.ForwardA) {
            case 0: operand1 = ID_EX_curr.read_data1; break;
            case 1: operand1 = wb_wr_data;            break;
            case 2: operand1 = EX_MEM_curr.alu_result;break;
        }
        switch (fwd.ForwardB) {
            case 0: forwarded_rt = ID_EX_curr.read_data2; break;
            case 1: forwarded_rt = wb_wr_data;            break;
            case 2: forwarded_rt = EX_MEM_curr.alu_result;break;
        }
        operand2 = ID_EX_curr.EX.ALUSrc ? ID_EX_curr.ext_imm : forwarded_rt;
        // EX_MEM_next.mem_wr_data = forwarded_rt;
        ...
        EX_MEM_next.mem_wr_data = forwarded_rt;
        uint32_t jr_target = ID_EX_curr.read_data1;
        if (fwd.ForwardA == 1) jr_target = wb_wr_data;
        else if (fwd.ForwardA == 2) jr_target = EX_MEM_curr.alu_result;
        ...
        uint32_t actual_target = EX_MEM_next.PC;
        ...
        if (ID_EX_curr.EX.Branch) {
            ...
            EX_MEM_next.PC = jr_target;
            actual_taken = true;
        } else if (ID_EX_curr.EX.Jump) {
            EX_MEM_next.PC = (ID_EX_curr.PC & 0xF0000000) | (ID_EX_curr.immj << 2);
            actual_taken = true;
        }
        EX_MEM_next.branch_taken = actual_taken;
        if (ID_EX_curr.EX.Branch || ID_EX_curr.EX.JR || ID_EX_curr.EX.Jump) {
            bp.update(ID_EX_curr.PC - 4, actual_taken, actual_taken?EX_MEM_next.PC:ID_EX_curr.PC, ID_EX_curr.EX.Jump || ID_EX_curr.EX.JR, ID_EX_curr.EX.Branch);
        }
        if(EX_MEM_next.branch_taken && EX_MEM_next.PC != ID_EX_curr.bh_PC){
            // printf("flush\n");
            IF_ID_next.reset();
            ID_EX_next.reset();
